...
public class HandingBUS {
    ...
    public static boolean checkInvaildTime(Timestamp start,Timestamp current ,Timestamp end)
    {
        if ((current.equals(start) && current.after(start)) || (current.equals(end) && current.before(end)))
        {
            return true;
        }
        return false;
